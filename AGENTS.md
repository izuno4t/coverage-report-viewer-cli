...
`jrv` is a Go CLI for browsing JaCoCo XML in the terminal.

- `cmd/jrv/main.go`: CLI entrypoint.
...
- `bin/jrv`: local built artifact (do not rely on this in commits).

## Build, Test, and Development Commands

- `go run ./cmd/jrv --help`: run the CLI locally.
- `go build ./cmd/jrv`: compile the binary for current OS/arch.
...
- `GOOS=linux GOARCH=amd64 go build -o /tmp/jrv-linux-amd64 ./cmd/jrv`: cross-build example.
