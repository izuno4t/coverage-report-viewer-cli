...
| F-IN-05 | 複数XMLをマージできる | M1-必須 | マルチモジュール検出、レポートマージ | TC-UNIT-012,TC-INT-009 |
| F-IN-06 | Cobertura XML を正規化できる | M1-必須 | Coberturaアダプタ | TC-UNIT-013 |
| F-IN-07 | 入力フォーマットを自動判別し `--format` 指定できる | M1-必須 | フォーマット判別、CLI formatオプション | TC-UNIT-014,TC-UNIT-015 |
| F-IN-08 | LCOV を正規化できる | M1-必須 | LCOVアダプタ | TC-UNIT-016,TC-INT-010 |
...
| F-NAV-06 | /で名前フィルターできる | M1-必須 | フィルタ入力モード、インクリメンタル絞り込み | TC-UNIT-017 |
| F-NAV-07 | g/Gで先頭末尾へジャンプできる | M1-必須 | 先頭/末尾ジャンプ操作 | TC-UNIT-018 |
| F-DSP-01 | 閾値に応じて色分け表示できる | M1-必須 | 色判定ロジック | TC-UNIT-010,TC-INT-007 |
| F-DSP-02 | カバレッジバーを表示できる | M1-必須 | バー生成・描画 | TC-UNIT-011,TC-INT-008 |
| F-DSP-03 | カウンタ種別切替ができる | M1-必須 | カウンタ切替描画 | TC-UNIT-019 |
| F-DSP-04 | 端末幅に応じて調整できる | M1-必須 | 可変バー幅・省略表示 | TC-UNIT-020 |
| F-DSP-05 | --no-colorを提供する | M1-必須 | NoColor描画制御 | TC-UNIT-021 |
